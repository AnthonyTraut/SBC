...
public class GraphCreator {
	...
	/**
	 * Permet de reset le graphe maintenu en m�moire
	 */
	 ...
	 * @param A2 : Recherche anc�tres et descendants
	 * @param A3 : Recherche les pr�dicats les plus instanci�es
	 * @return
	 */
	...
	public static void createGraph_A1(String classe, int limit) {
		...
		while (res.hasNext()) {
			...
			// V�rification que l'arc n'existe pas deja avant l'ajout
	...
	public static void createGraph_A3(String classe, int limit) {
		...
		// Afficher les predicats de la A-Box les plus instanci�es par des instances de la classe source
		QueryExecution queryexec = GraphCreator.execQuery(RequestBuilder.A3(classe, limit));
		ResultSet res = queryexec.execSelect();
		while (res.hasNext()) {
			QuerySolution sol = res.next();
			
			// Le nombre d'instances qui instancie le pr�dicat
